...
pub fn decode_trace(
    ...
    let (first_packet, _first_runtime_cfg) = packet::read_first_packet(&mut trace_reader)?;
    ...
    bus.broadcast(Entry::new_timed_event(Event::Start, first_packet.timestamp, pc, 0));
    ...
    loop {
        ...
        let get_insn_map = |p: Prv| -> &HashMap<u64, Insn> {
            insn_index.get(p)
        };
