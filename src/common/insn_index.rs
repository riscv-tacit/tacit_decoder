...
pub fn build_instruction_index(
    ...
    kernel_binary: &str,
    sbi_binary: &str,
    kernel_jump_label_patch_log: &str,
    ...
    let mut u_elf_file = File::open(application_binary)?;
    ...
    if u_insn_map.is_empty() {
        return Err(anyhow::anyhow!("No executable instructions found in app ELF"));
    }

    // Kernel-space instruction map
    let mut k_elf_file = File::open(kernel_binary)?;
    ...
    if k_insn_map.is_empty() {
        return Err(anyhow::anyhow!("No executable instructions found in kernel ELF"));
    }

    // Apply kernel jump label patch log
    let jump_label_patch_log = File::open(kernel_jump_label_patch_log)?;
    ...
    for (binary, entry) in driver_binary_entry_tuples {
    ...
    let mut m_elf_file = File::open(sbi_binary)?;
    ...
    if m_insn_map.is_empty() {
        return Err(anyhow::anyhow!("No executable instructions found in SBI ELF"));
    }
    debug!("[insn_index] found {} machine-space instructions", m_insn_map.len());
