        ...
        match entry.event {
            ...
            Event::UninferableJump => {
                ...
                if success && (frame_stack_size == 0) {
                    ...
                    debug!(
                        "Closing path on current path {:#x}",
                        curr_path_unwraped.addr
                    );
                    // if curr_path is contained in path_records, add the time interval to the record
                    if let Some(path_record) =
                        self.path_records.get_mut(&self.curr_path.as_ref().unwrap())
                    {
                        path_record.push(entry.timestamp.unwrap() - self.start_timestamp);
                    }
                    // otherwise, create a new record
                    else {
                        self.path_records.insert(
                            self.curr_path.as_ref().unwrap().clone(),
                            vec![entry.timestamp.unwrap() - self.start_timestamp],
                        );
                    }
                    self.path_time.push((
                        self.curr_path.as_ref().unwrap().clone(),
                        entry.timestamp.unwrap() - self.start_timestamp,
                    ));
                    self.curr_path = None;
                }
            }
    ...
    fn _flush(&mut self) {
        ...
        for (i, (path, time)) in self.path_time.iter().enumerate() {
            ...
            if i % 2 == 1 {
                ...
                self.writer
                    .write_all(format!("vq: {},", vq_str).as_bytes())
                    .unwrap();
                // time
                self.writer
                    .write_all(format!("time: {},", time).as_bytes())
                    .unwrap();
                // addr
                self.writer
                    .write_all(format!("PATH:{:#x}-", path.addr).as_bytes())
                    .unwrap();
