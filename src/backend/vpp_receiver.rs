                    ...
                    while self.curr_paths.len() > frame_stack_size {
                        ...
                        if let Some(path_record) = self.path_records.get_mut(&curr_path) {
                            ...
                            if self.use_bb_analysis {
                                ...
                                self.path_bb_records.get_mut(&curr_path).unwrap().push(
                                    bb_timestamp.iter().map(|&t| t - start_timestamp).collect(),
                                );
                            }
                        }
                        // otherwise, create a new record
                            ...
                            self.path_records.insert(
                                curr_path.clone(),
                                vec![entry.timestamp.unwrap() - start_timestamp],
                            );
                            if self.use_bb_analysis {
                                let bb_timestamp = self.bb_timestamps.pop().unwrap();
        ...
        for (path, intervals) in self.path_records.iter() {
            ...
            self.writer
                .write_all(
                    format!(
                        "INFO: {}: {}, line: {}\n",
                        symbol_info.name, symbol_info.file, symbol_info.line
                    )
                    .as_bytes(),
                )
                .unwrap();
            ...
            if self.use_bb_analysis {
                self.writer
                    .write_all(
                        format!(
                            "BB INTERVALS: {:?}\n",
                            self.path_bb_records.get(path).unwrap()
                        )
                        .as_bytes(),
                    )
                    .unwrap();
